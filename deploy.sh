...
# Always upload version.txt with no-cache headers (from build directory)
aws s3 cp build/version.txt s3://usublease-website/version.txt --cache-control "no-cache, no-store, must-revalidate"
cd ..
